...
object FormCoto {
  ...
  def init(id: String, autoSave: Boolean): (Model, Cmd.Single[Msg]) =
  ...
  case class Model(
    ...
    private def restoreCoto: Cmd.Single[Option[String]] = Cmd(IO {
  ...
  case class CotonomaForm(
    ...
    def validate(nodeId: Id[Node]): (CotonomaForm, Cmd.Single[Msg]) = {
      val (validation, cmd) =
