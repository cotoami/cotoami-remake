...
object CotonomaJson {
  def fetch(
      id: Id[Cotonoma]
  ): Cmd.Single[Either[ErrorJson, js.Tuple2[CotonomaJson, CotoJson]]] =
    Commands.send(Commands.Cotonoma(id))

  def fetchByName(
      name: String,
      nodeId: Id[Node]
  ): Cmd.Single[Either[ErrorJson, CotonomaJson]] =
    Commands.send(Commands.CotonomaByName(name, nodeId))

  def fetchRecent(
      nodeId: Option[Id[Node]],
      pageIndex: Double
  ): Cmd.Single[Either[ErrorJson, PaginatedJson[CotonomaJson]]] =
...
object CotonomaBackend {
  ...
  def fetch(id: Id[Cotonoma]): Cmd.Single[Either[ErrorJson, (Cotonoma, Coto)]] =
  ...
  def fetchByName(
      name: String,
      nodeId: Id[Node]
  ): Cmd.Single[Either[ErrorJson, Cotonoma]] =
  ...
  def post(
      ...
      location: Option[Geolocation],
      postTo: Id[Cotonoma]
  ): Cmd.Single[Either[ErrorJson, (Cotonoma, Coto)]] =
