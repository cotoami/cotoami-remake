//! Serialization codecs for inter-node communication.
//!
//! This module defines the transport-facing encoding rules used when
//! [`crate::remote::NodeSentEvent`] values are serialized and deserialized for
//! communication between cotoami nodes. Service-level payloads such as
//! [`crate::service::Request`] are carried within that event envelope, so these
//! codecs intentionally stay separate from the in-process domain types.
//!
...
//! - to decouple the external node-to-node protocol from in-process service
//!   and remote domain design
//! - to centralize compatibility decisions in one place
//! - to let multiple encodings such as JSON and MessagePack share the same
//!   logical event and request/command schema
//!
//! The typical flow is:
//!
//! - outgoing: internal event or payload type -> schema type -> serialized bytes
//! - incoming: serialized bytes -> schema type -> internal event or payload type
//!
